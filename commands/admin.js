	...
	execute(props) {
		...
		if (!msg.member.hasPermission('ADMINISTRATOR')) {
			return msg.channel.send(
				'You have to be an administrator to change server settings'
			)
		}
        ...
        apod_channel_id = VALUES(apod_channel_id)`
			...
			connection.query(query, (err) => {
				if (err) {
					...
					msg.channel.send(
						'There was an error setting the default channel for APOD'
					)
				} else {
					msg.channel.send(
						'Success! The NASA APOD will now be posted in this channel.'
					)
				}
			})
			return
        ...
        holiday_channel_id = VALUES(holiday_channel_id)`
			...
			connection.query(query, (err) => {
				if (err) {
					console.log('error showing table: ', err)
					msg.channel.send(
						'There was an error setting the default channel for holidays'
					)
				} else {
					msg.channel.send(
						'Success! Holidays will now be posted in this channel.'
					)
				}
			})
		}
